...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define CLK_FREQ     1  * pow(10,  6) // 1MHz
#define TIMER_PERIOD 50 * pow(10, -3) // 50 Milisecond
#define TIMER_FREQ   pow(TIMER_PERIOD, -1) // F = 1/T

#define PRESCALER 0     // (CLK_FREQ / TIMER_FREQ) / 2 - 1
#define AR_VALUE  49999 // (CLK_FREQ / TIMER_FREQ) / 2 - 1
...
void update_traffic(){
  ...
  if(timer_ticks >= 24 && traffic_state.current_led != &red_led){
    timer_ticks = 0;
