...
#define EXTI_ADDR PERIPHERAL_ADDR + 0x10400
...
#define GPIO_ADDR PERIPHERAL_ADDR + 0x10800
...
#define CORTEXM3_ADDR 0xE0000000
                      
#define NVIC_ADDR CORTEXM3_ADDR + 0xE100
...
traffic_state_t traffic_state = {&yellow_led, &yellow_led};
button_t button = {BUTTON_PIN, BUTTON_PORT, idle};
/* USER CODE END PV */
...
void button_state_update(button_t* button);
...
int main(void)
{
  ...
  gpio_pin_configure(button.port, button.pin, read);
...
void configure_interrupt(void){
  // Set Button Pin's Interrupt Mask to 1
  uint32_t* reg_imr = (uint32_t*) (EXTI_ADDR + EXTI_IMR_OFFSET);
  *reg_imr |= (1U << BUTTON_PIN);

  // Set the relevant FTSR Bit (Falling Edge) to 1
  uint32_t* reg_ftsr = (uint32_t*) (EXTI_ADDR + EXTI_FTSR_OFFSET);
  *reg_ftsr |= (1U << BUTTON_PIN);

  // Set the relevant RTSR (Rising Edge) Bit to 0
  uint32_t* reg_rtsr = (uint32_t*) (EXTI_ADDR + EXTI_RTSR_OFFSET);
  *reg_rtsr &= ~(1U << BUTTON_PIN);

  uint32_t* reg_afio = (uint32_t*) (AFIO_ADDR + AFIO_EXTICR3_OFFSET);
  *reg_afio |= (0x1 << (BUTTON_PIN % 4) * 4);
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
    ...
    if(traffic_state.current_led != &red_led) {traffic_state.previous_led = traffic_state.current_led;}
...
void update_traffic_light(traffic_state_t* traffic_state, button_t* button){
  ...
  button_state_update(button);

  if(traffic_state->current_led == &red_led){
    delay(500);
    // Extinguish the current led

    if(button->state == hold_down || button->state == pressed){return;}

    gpio_pin_toggle(traffic_state->current_led->port, traffic_state->current_led->pin);
    gpio_pin_toggle(traffic_state->previous_led->port, traffic_state->previous_led->pin);
    traffic_state->current_led = traffic_state->previous_led;
  ...
  if(traffic_state->current_led == &yellow_led){
